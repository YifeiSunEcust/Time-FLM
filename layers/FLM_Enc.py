...
class FLM_Enc(nn.Module):
    def __init__(self, config):
        super(FLM_Enc, self).__init__()
        self.input_embedding = DataEmbedding_wo_time(c_in=1, d_model=config.d_model, dropout=config.dropout)
        self.encoder_layer = nn.TransformerEncoderLayer(d_model=config.d_model, nhead=config.nhead)
        self.encoder = nn.TransformerEncoder(self.encoder_layer, num_layers=config.e_layers)
        self.fc = nn.Linear(config.d_model, 1)
    def forward(self, x, x_mask):
        batch_size, seq_pre_len, dim = x.size()
