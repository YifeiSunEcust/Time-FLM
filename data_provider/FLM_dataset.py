...
class FLM_Dataset(Dataset):
    def __init__(self, root_path, flag='train', total_range=None, pred_range=None,
                 file_pattern='*.csv', scale=True, percent=100):
        # 初始化参数
        self.total_range_min = total_range[0]
        self.total_range_max = total_range[1]
    ...
    def __read_multiple_files__(self):
        ...
        for file_path in csv_files:
            ...
            for batch_id in df_raw['batch_id'].unique():
                ...
                if len(batch_data) < self.total_range_max:
                    continue
                batches.append({
                    'data': batch_data,
        ...
        for dataset in self.datasets:
            ...
            train_end = int(0.5 * batch_num)
            test_end = train_end + int(0.1 * batch_num)
    ...
    def __getitem__(self, index):
        # total_len = random.randint(self.total_range_min, self.total_range_max)
        # pred_len = int(random.uniform(0.15, 0.5) * total_len)
        # seq_len = total_len - pred_len
    ...
    def __len__(self):
        total_len = 0
        for batch in self.select_batches:
            batch_len = len(batch['data']) - self.total_range_max + 1
            if batch_len > 0:
